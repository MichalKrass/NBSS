...
def read_data_info(dataset_info):
    for k, v in dataset_info.items():
        ...
        v['original_source'] = [str(p) for p in v['original_source']]
        # original_sources += v['original_source']
        v['wavname'] = k + '.wav'
        v['saveto'] = [k + '_mixture.wav']
        dataset_info[k] = v
    return dataset_info
...
class SmsWsjPlusDataset(Dataset):
    ...
    def __init__(
        ...
        # dataset_name_map = {"train_si284": "train", "cv_dev93": 'validation', 'test_eval92': 'test'}
        dataset_name_map = {'DNS': 'train'}
        ...
        {"datasets": 
            dataset_name: {
                id1: {audio_path : {original_source : []}},
                id2: {...},
            }}
        '''
        with open(self.sms_wsj_dir / 'DNS_clean.json', 'r') as f:
            d = json.load(f)
            self.dataset_info = d['datasets'][dataset]
            self.keys = list(self.dataset_info.keys())[:100]
            self.dataset_info = {self.keys[i] : self.dataset_info[self.keys[i]] for i in range(len(self.keys))}
        
        with open(self.sms_wsj_dir / 'DNS_noise.json', 'r') as f:
            d = json.load(f)
            self.dataset_info_noise = d['datasets'][dataset]
            self.keys_noise = list(self.dataset_info_noise.keys())[:100]
            self.dataset_info_noise = {self.keys_noise[i] : self.dataset_info_noise[self.keys_noise[i]] for i in range(len(self.keys_noise))}
        
        print("lengths of clean dataset: ", len(self.dataset_info), "lengths of noise dataset: ", len(self.dataset_info_noise))
        original_sources = []

        self.dataset_info = read_data_info(self.dataset_info)
        self.dataset_info_noise = read_data_info(self.dataset_info_noise)
        ...
        self.noises = list(v['original_source'][0] for v in self.dataset_info_noise.values())
        self.noises.sort()
        self.snr = snr

        self.rir_dir = Path(rir_dir).expanduser() / dataset_name_map[dataset]
    ...
    def __getitem__(self, index_seed: tuple[int, int]):
        ...
        for i in range(self.num_spk):
            ...
            # spk1 ans spk2 are same, for test
            uttrsc.append(info['original_source'][0])
            source, sr_src = sf.read(uttrsc[0], dtype='float32')
        ...
        #     lens=lens,
        # )
        # step 3 for enhancement: decide the length of the single-speaker mixture
        if self.audio_time_len is not None:
            mix_frames = int(self.audio_time_len * self.sample_rate)
        else:
            mix_frames = cleans[0].shape[0]
        lens = [mix_frames]
        ovlp_ratio = 0.0
        ovlp_type = None
        ...
        rvbts = rvbts[0][np.newaxis, :, :]
        targets = targets[0][np.newaxis, :, :]
        # print("rvbts, targets: ", rvbts.shape, targets.shape)
        ...
        # step 7: point-source noise
        # noise_path = self.noises[rng.integers(low=0, high=len(self.noises))]
        # noise_src, sr_noise = sf.read(noise_path, dtype='float32', always_2d=False)
        # assert sr_noise == self.sample_rate and noise_src.ndim == 1

        # noise_src = pad_or_cut([noise_src], lens=[mix_frames], rng=rng)[0]
...
class SmsWsjPlusDataModule(LightningDataModule):
    ...
    def __init__(
        self,
        sms_wsj_dir: str = './datasets/dns',  # a dir contains [early, noise, observation, rirs, speech_source, tail, wsj_8k_zeromean]
        rir_dir: str = './datasets/SMS_WSJ_Plus_rirs_3',  # containing train, validation, and test subdirs
        target: str = "direct_path",  # e.g. rvbt_image, direct_path
        datasets: Tuple[str] = ['DNS'],  # datasets for train/val/test/predict
        audio_time_len: Tuple[Optional[float], Optional[float], Optional[float], Optional[float]] = [10.0, 10.0, None, None],  # audio_time_len (seconds) for train/val/test/predictS
        ...
        num_spk: int = 1,  # separation task: 2 speakers; enhancement task: 1 speaker
